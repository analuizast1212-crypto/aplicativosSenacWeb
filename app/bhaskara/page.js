...
export default function Baskara() {
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 gap-6">
      ...
      {/* CARD */}
      <div className="bg-blue-400 p-6 rounded-2xl shadow-xl w-full max-w-sm">
        ...
        <div className="flex flex-col gap-2">
          <button
            onClick={calcular}
            className="w-full bg-blue-500 hover:bg-blue-700 text-white rounded p-2"
          >
            Calcular
          </button>
        ...
        {/* RESULTADO */}
        {passos.length > 0 && (
          <div className="mt-4 text-sm text-white bg-black p-3 rounded">
            {passos.map((p, i) => (
              <p key={i}>{p}</p>
            ))}
          </div>
